...
| One shared engine + working sync (10%) | **iOS app runs the SAME native `rslib` engine** (ios/rust-ffi → `mechgrader_engine_info` RPC shown in the Simulator) + **real sync round-trip** (`make sync-roundtrip`: A→self-hosted server→B, card_found=True) + sync conflict logic (10 tests). Remaining last-mile: physical-device signing + iOS sync button through the FFI. | `ios/rust-ffi/`, `mechgrader/tools/sync_roundtrip.py`, `mechgrader/sync/`, `docs/mobile.md` |
...
| No engine-sharing syncing phone → max 70% | **Substantially addressed** — the iOS app runs the **native `rslib` engine** in the Simulator (`make ios`, engine-info RPC proven) and sync works end-to-end (`make sync-roundtrip`, A→server→B). Remaining: physical-device signing + wiring the iOS sync button through the FFI (`docs/mobile.md`). |
